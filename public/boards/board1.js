(function() {
      ...
      function handleShot(e) {
        ...
        // Эффект взрыва – теперь точно по координатам клика (центрируется через CSS transform)
      ...
      function createShotEffect(pixelX, pixelY) {
        ...
        // Теперь позиционируем точно, используя transform: translate(-50%, -50%) в CSS
        img.style.left = pixelX + 'px';
        img.style.top = pixelY + 'px';
        gameArea.appendChild(img);
        // через 2 секунды начать затухание
        setTimeout(() => img.classList.add('fading'), 2000);
        // удаление после перехода (1с)
